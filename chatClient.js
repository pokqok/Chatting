...
window.onload = function () {
  ...
  var smallRooms = document.querySelector("#smallRooms");
  var bigRooms = document.querySelector("#bigRooms");
  DATA.map((item) => {
    var roomButton = document.createElement("div");
    var roomLink = document.createElement("a");

    roomLink.href = `http://127.0.0.1:3000/?room_name=${item.room_name}`;
    roomLink.innerText = item.room_name;
    roomLink.style.textDecoration = "none";
    roomLink.style.color = "white";
    roomLink.style.fontWeight = "bold";
    roomButton.style.width = "70%";
    ...
    roomButton.style.background = "black";
    if (item.linked_id) {
      bigRooms.appendChild(roomButton);
    } else {
      smallRooms.appendChild(roomButton);
    }
  });
  ...
  socket.on("inviteAllInRoom", function (data) {
    ...
    socket.emit(
      "inviteAllInRoomSuccess",
      "방의 모든 참가자에게 초대 메시지를 전송했습니다."
    );
  });
